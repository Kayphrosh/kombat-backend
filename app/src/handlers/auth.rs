...
pub async fn verify_dynamic(
...
) -> Result<Json<crate::models::DynamicAuthResponse>, (StatusCode, Json<crate::models::ApiResponse<()>>)> {
    let dynamic_svc = state.dynamic_service.as_ref()
        .ok_or_else(|| (StatusCode::INTERNAL_SERVER_ERROR, Json(crate::models::ApiResponse::err("Dynamic service not configured"))))?;

    // Verify the Dynamic SDK token
    let claims = dynamic_svc.verify_token(&req.dynamic_token)
        .await
        .map_err(|e| (StatusCode::UNAUTHORIZED, Json(crate::models::ApiResponse::err(format!("Dynamic token verification failed: {}", e)))))?;

    // Extract wallet address from the token claims
    let wallet = claims.wallet_address()
    ...
    let user = state.db.upsert_user(&wallet, &profile_req)
        .await
    ...
    let jwt_claims = Claims { wallet: &wallet, exp };
    let secret = std::env::var("AUTH_JWT_SECRET")
        .map_err(|_| (StatusCode::INTERNAL_SERVER_ERROR, Json(crate::models::ApiResponse::err("server missing AUTH_JWT_SECRET"))))?;
    let token = jsonwebtoken::encode(&Header::default(), &jwt_claims, &EncodingKey::from_secret(secret.as_bytes()))
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(crate::models::ApiResponse::err(format!("jwt encode error: {}", e)))))?;
